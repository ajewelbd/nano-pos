...
class Sales
{
	...
	public function submit_order($orders_info)
	{
		...
		if (!empty($products) && is_array($products) && !empty($others)) {
			...
			$payment_info = [
				'type' => 'sale',
				'entry_type' => 1,
				'customer_id' => $others->customer_id,
				'amount' => $others->total_payable,
				'paid' => $others->paid,
				'remarks' => $sales_id,
				'added_by' => $others->uid
			];
			$submitCustomerPayment = $customer->customerPaymentUpdate($payment_info);
			if ($sales_entry && count($products) > 0 && $submitCustomerPayment['state'] == 'OK') {
				for ($i = 0; $i < count($products); $i++) {
					...
					$update_inventory = $inventory->changeQuantity($inventory_data, $products[$i]->inv_id);
					//var_dump($update_inventory);die();
					if ($update_inventory['state'] == 'OK') {
	...
	{
		//$sql="SELECT * FROM sales WHERE active=1 AND deleted IS NULL ORDER BY created DESC";
		$sql = "SELECT sales.id, sales.invoice, sales.total, sales.paid, sales.due, sales.payment_method, sales.created, customers.name AS customer_name FROM sales 
		INNER JOIN customers ON customers.id=sales.customer_id WHERE sales.active=1 AND sales.deleted IS NULL ORDER BY created DESC";
	...
	{
		$sql = "SELECT products.id, products.product_name, products.base_price, inventory.id AS inv_id, inventory.quantity FROM products INNER JOIN inventory ON products.id=inventory.product_id WHERE products.active=1";
		$query = $this->db->prepare($sql);
		$query->execute();
